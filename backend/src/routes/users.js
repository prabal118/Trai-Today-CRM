...
export async function handleUserRoutes({ req, res, path, sendJSON }) {
  ...
  if (path === "/api/users" && req.method === "POST") {
    ...
    const taken = db.prepare("SELECT 1 FROM users WHERE username = ?").get(username.trim());
    ...
    ).run(id, name.trim(), username.trim(), hashPassword(password), role);
    writeAudit({ userId: req.user.sub, action: "create", module: "users", recordId: id, newValue: { name, username, role } });
