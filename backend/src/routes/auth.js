...
export async function handleAuthRoutes({ req, res, path, sendJSON, SESSION_SECRET }) {
  ...
  if (path === "/api/auth/bootstrap" && req.method === "POST") {
    ...
    const user = transaction(() => {
      ...
      ).run(id, name.trim(), username.trim(), hashPassword(password));
      db.prepare(`INSERT INTO security (id, recovery_phrase_hash) VALUES (1, ?)`).run(hashPassword(recoveryPhrase.trim()));
      writeAudit({ userId: id, action: "create", module: "users", recordId: id, newValue: { name, username, role: "admin" } });
  ...
  if (path === "/api/auth/login" && req.method === "POST") {
    ...
    if (row.role === "employee" && row.employee_code) {
      const emp = db.prepare("SELECT status FROM employees WHERE code = ?").get(row.employee_code);
